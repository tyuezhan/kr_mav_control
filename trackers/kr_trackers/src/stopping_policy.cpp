...
void StoppingPolicy::Initialize(const ros::NodeHandle& nh) {
  ...
  priv_nh.param("acc_xy_des", a_xy_des_, 4.0);
  priv_nh.param("jerk_xy_des", j_xy_des_, 6.0);
  priv_nh.param("acc_z_des", a_z_des_, 3.0);
  priv_nh.param("jerk_z_des", j_z_des_, 3.0);
...
PositionCommand::ConstPtr StoppingPolicy::update(
  ...
  cmd->yaw =cmd_pos_(3);
  cmd->yaw_dot = cmd_vel_(3);
  ROS_INFO_STREAM_THROTTLE(1.0, "Stopping policy updated, current cmd pos is: " << cmd_pos_(0)
                                                                  << " "
                                                                  << cmd_pos_(1)
                                                                  << " "
                                                                  << cmd_pos_(2)
